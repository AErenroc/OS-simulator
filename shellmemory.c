...
// Our API is to allocate one line at a time. Normally, we'd have
...
//
// There is one case of 'recursive' exec: the tests that look like
//   exec .... #
//   echo shell
//   exec ....
...
// We simply record the next free index in a variable.
...
// linememory is completely empty at certain times. For sanity checking, use an additional function to 
// assert that the the linememory is empty.
...
// We made init_mem call init_linemem, below.
void init_linemem() {
    for (size_t i = 0; i < MEM_SIZE; ++i) {
...
// Return a const pointer to ensure the caller doesn't do something bad,
// like try to free it.
...
// [Key-value memory for variables ]------------------------------------------------------------------------------------
