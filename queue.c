...
struct queue {
    ...
    // For now --> No tail as there's only ever 4 items in our queue.
    // Dereferencing 4 pointers is not to slow.
    // TODO: implement the bookkeeping for a tail pointer
    //struct PCB *tail;
};
...
void enqueue_fcfs(struct queue *q, struct PCB *pcb) {
    // NOTE: some dequeue operation didn't do its job if this isn't NULL
...
void enqueue_aging(struct queue *q, struct PCB *pcb) {
    ...
    // If we always break ties at the head like any other tie, then we will
    ...
    // OR we can instead do this: we know that programs that are
