...
int interpreter(char *command_args[], int args_size) {
    ...
    if (args_size < 1) { 
        // spec says we shouldn't just ignore
        // blank lines. In  real implementation,
...
// or by checking the first character when printing
// the names. But this is more robust.
// And is also better since it won't allocate extra dirents.
...
int ls() {
    ...
    if (n == -1) {
        // something is very wrong, just give up.
...
int cd(char *path) {
    ...
    if (result) {
        // TODO: chdir can fail for several reasons, but the only one we need
        ...
        // first while two users are on the system theres a race condition.
        return badcommandCd();
    }
    return 0;
}
...
int my_exec(char *args[], int args_size) {
    ...
    for (int n = 0; n < args_size; ++n) {
        ...
        // ---------------------------------------------
        // We're going to assume that for this check, 'recursive exec' is not involved. 
        // Obviously this doesn't hold in a real OS.
        // Having a proper process table, rather than only a schedule, would solve the problem.
...
background_cleanup:
    ...
    //   exec P3 P1
    //   quit
    // The schedule will end up containing P1, P2, and P3, because the
    // background exec will refuse to allocate a second PCB for P1 but
    // will not de-allocate the PCB it already made for P3.
    // we may handle corner cases however we like, so that's fine.
cleanup:
...
int run(char *argv[], int args_size) {
    ...
    // of args. we just copy it here, into an array that we are certain, locally, has space.
